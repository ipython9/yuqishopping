...
public class OrderServiceImpl implements OrderService {
    ...
    public List<Order> orderList(String name){
        ...
        for (TbOrder t:tbOrderList) {
            ...
            order.setReceiveaddress(t.getReceiver());
            order.setReceiveaddress(t.getReceiverAreaName());
            order.setReceivemobile(t.getReceiverMobile());
            //通过tborderitem  进行设置值
    ...
    public PageResult findPage(int pageNum, int pageSize,String name) {
        ...
        criteria.andSellerIdEqualTo(name);
        List<TbOrder> tbOrderlist = orderMapper.selectByExample(tbOrderExample);
        PageInfo orderPageInfo = new PageInfo(tbOrderlist);
        orderPageInfo.setList(orderList);
        orderPageInfo.setTotal(tbOrderlist.size());
